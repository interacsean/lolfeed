...
export const extractTime = (rawDesc: string) => {
  ...
  const isDateRangeNoTime = rawDesc.match((/^(.*?)\s*[-–]\s*(\w*?)\s*(\d+)\s*[-–—]\s*(\d+)$/))
  const [, desc, month, daySt, dayFn]: (string | null)[] = isDateRangeNoTime || Array(5).fill(null);
  if (desc && daySt && month && dayFn) {
    const timestamp = [
      parseFromString(`${daySt.padStart(2, '0')} ${replaceMonthWithNumeric(month)} ${year} 12:00PM`, 'DD MM YYYY H:mmA').getTime(),
      parseFromString(`${dayFn.padStart(2, '0')} ${replaceMonthWithNumeric(month)} ${year} 12:00PM`, 'DD MM YYYY H:mmA').getTime(),
    ];
    return [
      desc,
      timestamp,
  ...
  const isTimeOfDate = rawDesc.match((/^(.*?)\s*[-–]\s*(\w*?)\s*(\d+)\s*(\w*?)\s*[-–]?\s?([\d\.:]+)(am|pm)$/i))
  const [, desc2, , day2, month2, time, ampm]: (string | null)[] = isTimeOfDate || Array(7).fill(null);
  if (desc2 && day2 && month2 && time && ampm) {
    const timeWColon = time.replace('.', ':')
    const timeWithMins = timeWColon.includes(':') ? timeWColon : `${timeWColon}:00`
    const dateString = `${day2.padStart(2, '0')} ${replaceMonthWithNumeric(month2)} ${year} ${timeWithMins}${ampm.toUpperCase()}`;
    const timestamp =
  ...
  const isTimeOfDateRev = rawDesc.match((/^(.*?)\s*[-–]\s*(\w*?)\s*(\w*?)\s*(\d+)\s*[-–]?\s*([\d\.:]+)(am|pm)$/i))
  const [, desc3, , month3, day3, time3, ampm3]: (string | null)[] = isTimeOfDateRev || Array(7).fill(null);
  if (desc3 && day3 && month3 && time3 && ampm3) {
    const timeWColon = time3.replace('.', ':')
    const timeWithMins = timeWColon.includes(':') ? timeWColon : `${timeWColon}:00`
    const dateString = `${day3.padStart(2, '0')} ${replaceMonthWithNumeric(month3)} ${year} ${timeWithMins}${ampm3.toUpperCase()}`;
    // return dateString;
    const timestamp =
      [parseFromString(dateString, 'DD MM YYYY H:mmA').getTime()];
    return [
