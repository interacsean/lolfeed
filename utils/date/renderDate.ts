...
const renderDate = (
  ...
  const start = dateFnsTz.convertToTimeZone(times[0], { timeZone });
  const end = !times[1]
    ? null
    : dateFnsTz.convertToTimeZone(times[1], { timeZone });
  ...
  const sameDate =
    end && sameMonth && sameYear && start.getDate() === end.getDate();
  ...
  const showEndTime = precision === TimestampPrecision.TIME && end && sameDate;
  const showEndYear = !thisYear && end;
  const showEndMonth = !thisMonth && end;

  const startTimeFmt =
    showStartTime && format(start, `h:mm${showEndTime ? '' : 'aaa'}`);

  const endTimeFmt = showEndTime && format(end, `-h:mmaaa`);

  const endsOnDifferentDate = end && !sameDate;
  const startDateFmt =
    isToday && !endsOnDifferentDate
      ? `To${start.getHours() > 17 ? 'night' : 'day'}`
      : format(
          start,
          `EEE do${showStartMonth ? ' MMM' : ''}${
            showStartYear ? ' yyyy' : ''
          }`,
        );
  ...
  return `${startTimeFmt || ''}${endTimeFmt || ''} ${startDateFmt}${
    endDateFmt || ''
  }`;
};
